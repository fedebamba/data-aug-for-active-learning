...
def a_single_experiment(esname, esnumber):
    ...
    for i in range(first_time_multiplier, until_slice_number):
        ...
        print("ACTIVE:")
        best_act_net, best_act_acc = single_train_batch(num_of_epochs=epochs_second_step, dataset=dataset, indices=el_for_active, name="results_{0}_{1}_act".format(esname, esnumber))
